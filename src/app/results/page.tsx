...
export default function Results() {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      // Check for cached result to prevent re-analysis on refresh
      if (cachedResult) {
  ...
  const speakFeedback = () => {
    if (!data) return;
    window.speechSynthesis.cancel();
    if (isSpeaking === "feedback") {
      setIsSpeaking(null);
      return;
    }
    const utterance = new SpeechSynthesisUtterance(data.feedback_summary);
    utterance.rate = 0.9;
    utterance.onstart = () => setIsSpeaking("feedback");
  ...
  const speakIdeal = () => {
    ...
    const utterance = new SpeechSynthesisUtterance(data.better_version);
    utterance.rate = 0.95;
    utterance.onstart = () => setIsSpeaking("ideal");
    utterance.onend = () => setIsSpeaking(null);
    window.speechSynthesis.speak(utterance);
  };
      ...
      <div className="max-w-6xl mx-auto" id="results-content">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          <div className="lg:col-span-8 space-y-10">
            ...
            {/* A Better Way to Say It - Cinematic Card */}
            <motion.section 
              initial={{ y: 30, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
