        ...
        <header className="absolute top-0 left-0 right-0 z-50">
          <div className="max-w-7xl mx-auto px-6 md:px-8 h-20 md:h-24 flex items-center justify-between">
            <Link href="/" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
              ...
              <span className="text-xl font-black tracking-tighter text-white">SpeakMirror</span>
            </Link>

            <div className="flex items-center gap-4">
              <ThemeToggle />
              {user ? (
                <Link href="/dashboard" className="px-5 md:px-6 py-2 rounded-xl bg-white text-[#0A1A2F] font-bold text-sm hover:bg-[#FDFBF7] transition-colors shadow-xl">
                  Dashboard
                </Link>
              ) : (
                <button
                  onClick={() => setIsAuthModalOpen(true)}
                  className="px-5 md:px-6 py-2 rounded-xl bg-white text-[#0A1A2F] font-bold text-sm hover:bg-[#FDFBF7] transition-colors shadow-xl"
                >
                  Sign In
                </button>
              )}
